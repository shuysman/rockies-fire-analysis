...
setwd("C:\\David\\Water balance\\GRSA\\Paper folder\\Fire\\")
srmaster<-read.csv("SR_master.csv",header=TRUE, stringsAsFactors=FALSE);head(srmaster)
#due to small shrub sample size n=10 group shrub and herb into non-forest type
unique(srmaster$maj_class)
a<-which(srmaster$maj_class=="Herb ");a
srmaster$maj_class[a]<-"non-forest"
a<-which(srmaster$maj_class=="Shrub");a
srmaster$maj_class[a]<-"non-forest"
a<-which(srmaster$maj_class=="Tree ");a
srmaster$maj_class[a]<-"forest"
unique(srmaster$maj_class)

#build some charts of fire data showing histogram of fire doy to determine fire season and histogram of size
dtx<-as.Date(srmaster$ig_date, format = "%m/%d/%Y");dtx#format date as POSIX so you and add and subtract number of days that bracket time period of interest
...
srmasterwf<-subset(srmaster, srmaster$Fire_Type=="WF");head(srmasterwf)
...
nonforest<-subset(srmasterwf, maj_class=="non-forest");nonforest#note class Tree has a space at end
...
forest_end<-as.Date(doy_min_max[1,2], origin = "2015-01-01");forest_end
nonforest_end<-as.Date(doy_min_max[2,2], origin = "2015-01-01");nonforest_end
...
sites<-unique(srmasterwf$Fire_ID);sites
site_list<-NULL
igdates<-NULL
n=1
for (n in 1:length(sites)){
  site<-srmasterwf[n,1];site
  site_list[n]<-site
  igdate<-srmasterwf[n,16]
  igdates[n]<-igdate
 }
...
setwd("C:\\David\\Water balance\\GRSA\\Paper folder\\Fire\\daily\\")
n= 2
##########################################################################################################################
...
for (n in 1:nrow(igsite)){
  ...
  dt<-igsite[n,2];dt
  dtx<-as.Date(dt, format = "%m/%d/%Y");dtx#format date as POSIX so you and add and subtract number of days that bracket time period of interest
  ...
  names(sitewb)
  corewb<-sitewb[,c(-1,-24)];head(corewb)#drop dates and strings prior to converting to xts
  ...
  awssm<-100-wbxts$SOIL;head(awssm);names(awssm)<-"AWSSM"#  this should be added to water balance model
  ...
  a<- which(srmasterwf$site == site);a#identifies the row number in site file attribute file
  names(srmasterwf)
  ...
  #determine average conditions for the 7 days prior to ignition
   ...
   period_means<-rollapply(historic[,c("SVP","RD","VPD","TEMP","SOIL","GDD","AWSSM")], 14, mean, by = 1, by.column = TRUE);period_means[1:16,]#rolling values start at doy 14
   period_sums<-rollapply(historic[,c("RAIN","AET","D")], 14, sum, by = 1, by.column = TRUE)
  ...
  ig_status<-wbxts[dtx, c("SVP","RD","VPD","P","TEMP","RAIN","PACK","W","PET","W_PET","SOIL","SOIL_Delta","AET","RUNOFF","D","GDD","AWSSM")];head(ig_status)
  mean_ig_status<-colMeans(ig_status[,c("SVP","RD","VPD","TEMP","SOIL","D","GDD","AWSSM")]);mean_ig_status
  names(mean_ig_status)[1:8]<-c("meanSVP","meanRD","meanVPD","meanTEMP","meanSOIL","meanD","meanGDD","meanAWSSM");mean_ig_status#rename the columns
  ...
  max_ig_status<-apply(ig_status[,c("VPD","TEMP","GDD","AWSSM")],2,max);max_ig_status
  names(max_ig_status)[1:4]<-c("maxVPD","maxTEMP","maxGDD","maxAWSSM");max_ig_status
  min_ig_status<-apply(ig_status[,c("SVP","RD","SOIL")],2,min);min_ig_status
  names(min_ig_status)[1:3]<-c("minSVP","minRD","minSOIL");min_ig_status
  #aggregate into a data frame means or sums as appropriate depending on nature of variable.  generally water should be summed, temperature and pressure averaged
  ...
  ig_end<-wbxts[paste(ig_date,ig_cal_end, sep="/"), c("SVP","RD","VPD","P","TEMP","RAIN","PACK","W","PET","W_PET","SOIL","SOIL_Delta","AET","RUNOFF","D","GDD","AWSSM")];head(ig_end)
  mean_ig_end<-colMeans(ig_end[,c("SVP","RD","VPD","TEMP","SOIL","D","GDD","AWSSM")]);mean_ig_end
  names(mean_ig_end)[1:8]<-c("meanSVP","meanRD","meanVPD","meanTEMP","meanSOIL","meanD","meanGDD","meanAWSSM");mean_ig_end#rename the columns
  sum_ig_end<-colSums(ig_end[,c("RAIN","AET","D","GDD")]);sum_ig_end
  names(sum_ig_end)[1:4]<-c("sumRAIN","sumAET","sumD","sumGDD");sum_ig_end
  max_ig_end<-apply(ig_end[,c("VPD","TEMP","GDD","AWSSM")],2,max);max_ig_end
  names(max_ig_end)[1:4]<-c("maxVPD","maxTEMP","maxGDD","maxAWSSM");max_ig_end
  min_ig_end<-apply(ig_end[,c("SVP","RD","SOIL")],2,min);min_ig_end
  names(min_ig_end)[1:3]<-c("minSVP","minRD","minSOIL");min_ig_end
  #aggregate into a data frame means or sums as appropriate depending on nature of variable.  generally water should be summed, temperature and pressure averaged
  ...
  cur_yr<-wbxts[paste(ig_cal_start,ig_cal_end, sep="/"), c("SVP","RD","VPD","P","TEMP","RAIN","PACK","W","PET","W_PET","SOIL","SOIL_Delta","AET","RUNOFF","D","GDD","AWSSM")];head(cur_yr)
  mean_cur_yr<-colMeans(cur_yr[,c("SVP","RD","VPD","TEMP","SOIL","D","GDD","AWSSM")]);mean_cur_yr
  names(mean_cur_yr)[1:8]<-c("meanSVP","meanRD","meanVPD","meanTEMP","meanSOIL","meanD","meanGDD","meanAWSSM");mean_cur_yr#rename the columns
  sum_cur_yr<-colSums(cur_yr[,c("RAIN","AET","D","GDD")]);sum_cur_yr
  names(sum_cur_yr)[1:4]<-c("sumRAIN","sumAET","sumD","sumGDD");sum_cur_yr
  max_cur_yr<-apply(cur_yr[,c("VPD","TEMP","GDD","AWSSM")],2,max);max_cur_yr
  names(max_cur_yr)[1:4]<-c("maxVPD","maxTEMP","maxGDD","maxAWSSM");max_cur_yr
  min_cur_yr<-apply(cur_yr[,c("SVP","RD","SOIL")],2,min);min_cur_yr
  names(min_cur_yr)[1:3]<-c("minSVP","minRD","minSOIL");min_cur_yr
  #aggregate into a data frame means or sums as appropriate depending on nature of variable.  generally water should be summed, temperature and pressure averaged
  ...
  lag_one<-wbxts[paste(lag1_start,lag1_end, sep="/"), c("SVP","RD","VPD","P","TEMP","RAIN","PACK","W","PET","W_PET","SOIL","SOIL_Delta","AET","RUNOFF","D","GDD","AWSSM")];head(lag_one)
  mean_lag_one<-colMeans(lag_one[,c("SVP","RD","VPD","TEMP","SOIL","D","GDD","AWSSM")]);mean_lag_one
  names(mean_lag_one)[1:8]<-c("lag1meanSVP","lag1meanRD","lag1meanVPD","lag1meanTEMP","lag1meanSOIL","lag1meanD","lag1meanGDD","lag1meanAWSSM");mean_lag_one#rename the columns
  ...
  max_lag_one<-apply(lag_one[,c("VPD","TEMP","GDD","AWSSM")],2,max);max_lag_one
  names(max_lag_one)[1:4]<-c("lag1maxVPD","lag1maxTEMP","lag1maxGDD","lag1maxAWSSM");max_lag_one
  min_lag_one<-apply(lag_one[,c("SVP","RD","SOIL")],2,min);min_lag_one
  names(min_lag_one)[1:3]<-c("lag1minSVP","lag1minRD","lag1minSOIL");min_lag_one
  #aggregate into a data frame means or sums as appropriate depending on nature of variable.  generally water should be summed, temperature and pressure averaged
 ...
 if(attributes$maj_class=="forest"){
   ...
   ll <- seq(forest_end, forest_start, by = "-1 day");ll#find ig doy's in years before ignition
    ...
    doy_ann_sub<-doy_ann[,c("year","month","day","SVP","RD","VPD","TEMP","SOIL","GDD","AWSSM","RAIN","AET","D")];head(doy_ann_sub)
   ...
   #compute percentile rank of each observation in the sequence of years 
    ...
    ranks<-apply(rankthis_forest[5:14],2,percent_rank)
    ...
    ranks<-apply(rankthis_nonforest[5:14],2,percent_rank)
    ranksxts<-xts(coredata(ranks),order.by=f3);head(ranksxts)#build xts data structure 
    #igstatus<-as.data.frame(wbxts[ig_date]);igstatus
    igrank<-as.data.frame(ranksxts[ig_date]);igrank
    igrankatt<-cbind(igrank, attributes)
...
if(attributes$maj_class=="forest"){
  ...
  fire_forest<-cbind(attributes$site,attributes$maj_class,fire_forest);head(fire_forest)#add site name as a key for look up if needed later
  ...
  fire_nonforest<-cbind(attributes$site,attributes$maj_class,fire_nonforest);head(fire_nonforest)#add site name as a key for look up if needed later
...
forest_svp_roc<-roc(fire_forest_long$fire,fire_forest_long$SVP, plot=TRUE, legacy.axes=TRUE, percent=TRUE, xlab="False Positive Percentage", ylab="True Postive Percentage", col="#377eb8", lwd=4, print.auc=TRUE)#build roc
...
forest_temp_roc<-roc(fire_forest_long$fire,fire_forest_long$TEMP, plot=TRUE, legacy.axes=TRUE, percent=TRUE, xlab="False Positive Percentage", ylab="True Postive Percentage", col="#377eb8", lwd=4, print.auc=TRUE)
...
legend(35, 75, legend=c(paste("RD ",round(auc(forest_rd_roc),1)),
       ...
       bty="n", col=c("black", "dark blue","green","red","brown","purple","dark green","orange"), #col=c("black","blue", "dark blue","green","red","brown","magenta","purple","dark green","orange")
...
nonforest_svp_roc<-roc(fire_nonforest_long$fire,fire_nonforest_long$SVP, plot=TRUE, legacy.axes=TRUE, percent=TRUE, xlab="False Positive Percentage", ylab="True Postive Percentage", col="#377eb8", lwd=4, print.auc=TRUE)#build roc
...
nonforest_temp_roc<-roc(fire_nonforest_long$fire,fire_nonforest_long$TEMP, plot=TRUE, legacy.axes=TRUE, percent=TRUE, xlab="False Positive Percentage", ylab="True Postive Percentage", col="#377eb8", lwd=4, print.auc=TRUE)
...
roc(fire_nonforest_long$fire,fire_nonforest_long$TEMP, main="TEMP",plot=TRUE, legacy.axes=TRUE, percent=TRUE,
    xlab="False Positive Percentage", ylab="True Postive Percentage", col="#377eb8", lwd=4, print.auc=TRUE,
    print.auc.x=95, partial.auc=c(100, 80), auc.polygon = TRUE, auc.polygon.col = "#377eb822")
...
roc(fire_forest_long$fire,fire_forest_long$TEMP, main="TEMP",plot=TRUE, legacy.axes=TRUE, percent=TRUE,
    xlab="False Positive Percentage", ylab="True Postive Percentage", col="#377eb8", lwd=4, print.auc=TRUE,
    print.auc.x=95, partial.auc=c(100, 80), auc.polygon = TRUE, auc.polygon.col = "#377eb822")
...
y<-as.vector(ranksnon[,"halog"]);y
xcann<-subset(ranksnon,select=c("TEMP","SOIL","GDD","AWSSM","AET","D","SVP","RD","VPD"));head(xcann)#
hier.part(y,xcann,barplot=TRUE)

ranksforest<- subset(rankstack, maj_class=="forest");head(rankstack);names(rankstack)
y<-as.vector(ranksforest[,"halog"]);y
xcann<-subset(ranksforest,select=c("TEMP","SOIL","GDD","AWSSM","AET","D","SVP","RD","VPD"));head(xcann)#
hier.part(y,xcann,barplot=TRUE)
...
ncol(rankstack2)
colnames(rankstack2)[17]<-"burned";head(rankstack2)
...
#c("SVP","RH","VPD","TEMP","SOIL","GDD","AWSSM","AET","D")
...
rankforest<-subset(rankstack, select=c("site", "maj_class", "doy","halog", "SVP", "RD", "VPD", "SOIL", "GDD", "TEMP","AWSSM", "AET", "D"));head(rankforest)
mquant<-melt(rankforest,id=c("maj_class","site","doy","halog"));head(mquant)
ggplot(mquant, aes(x=value, y=halog, colour=factor(maj_class))) +geom_point(size=3)+ #geom_vline(xintercept = 0.6, linetype="solid",color = "blue", size=0.5)+
  #geom_smooth(method="nls", formula = 'y ~ a*exp(b*x)',method.args = list(start=list(a=.1,b=.5)),se = FALSE)+
...
mranks<-melt(ranks2, id=c("doy","site","maj_class","Fire_Type","halog"));head(mranks)
# Multiple ECDFs
...
head(rankstack)
  ...
  full_climvars<-c("SVP","RD","VPD","TEMP","SOIL","GDD","AWSSM","AET","D")
  ...
  for (t in c("VPD","SOIL","AWSSM","D")){
    ...
    if (t=="P"|t=="ET"|t=="SOIL"||t=="D"|t=="SVP"|t=="VPD"|t=="GDD"||t=="AWSSM"){#"SVP","RD","VPD","TEMP","SOIL","GDD","AWSSM","AET","D"
      my.equation<-halog~a*exp(b*value)# where exp is 2.781 
...
mrankstack<-melt(rankstack2, id=c("doy","site","maj_class","Fire_Type","halog"));head(mrankstack)
...
mean_rank<-apply(burn_ranks[,c("SVP","RH","VPD","TEMP","SOIL")],2,mean);mean_rank
max_rank<-apply(burn_ranks[,c("SVP","RH","VPD","TEMP","SOIL")],2,max);max_rank
min_rank<-apply(burn_ranks[,c("SVP","RH","VPD","TEMP","SOIL")],2,min);min_rank#need to include data outside of ig date or this can be zero
median_rank<-apply(burn_ranks[,c("SVP","RH","VPD","TEMP","SOIL")],2,median);median_rank
...
lines(Data$x, predict(nls_fit), col = "red")
  ...
  climout365<-wbxts[paste(dtx365,dtx, sep="/"), c("SVP","RH","VPD","P","TEMP","RAIN","PACK","W","PET","W_PET","SOIL","SOIL_Delta",
                                              "AET","RUNOFF","D","GDD")];head(climout365)
 
  
  dtx7<-as.Date(dtx)-7;dtx7#7days prior to ignition
  climout7<-wbxts[paste(dtx7,dtx, sep="/"), c("SVP","RH","VPD","P","TEMP","RAIN","PACK","W","PET","W_PET","SOIL","SOIL_Delta",
  ...
  names(corewb)
  climout14<-wbxts[paste(dtx14,dtx, sep="/"), c("SVP","RH","VPD","P","TEMP","RAIN","PACK","W","PET","W_PET","SOIL","SOIL_Delta",
                                              "AET","RUNOFF","D","GDD")];climout14
  
  #apply means or sums as appropriate depending on metric.
  mean7<-apply(climout7[,c("SVP","RH","VPD","TEMP","SOIL")],2,mean)
