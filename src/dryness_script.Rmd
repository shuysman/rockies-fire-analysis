...
ggplot() +
  ...
  geom_sf(data = ecoregion, inherit.aes = FALSE, alpha = 0.25, fill = "gold") +
...
make_roc <- function(options) {
  ...
  png(filename = paste0("/home/steve/OneDrive/northern-rockies-dryness/img/", region_name, "-", cover_class, "-", roll_window, "-days.png"), width = 600, height = 600)
...
### Window vs AUC
``` {r auc-window, fig.height = 10, fig.width = 7}
...
``` {r forest-dens, fig.height = 7, fig.width = 7}
wbdata_percentiles_forest %>%
  filter(window == 3) %>%
  mutate(fire = factor(fire)) %>%
  pivot_longer(cols = c(RD, VPD, T, SOIL, AWSSM, RAIN, AET, D, GDD)) %>%
  ggplot() +
...
``` {r non_forest-dens, fig.height = 7, fig.width = 7}
wbdata_percentiles_non_forest %>%
  filter(window == 3) %>%
  mutate(fire = factor(fire)) %>%
  pivot_longer(cols = c(RD, VPD, T, SOIL, AWSSM, RAIN, AET, D, GDD)) %>%
