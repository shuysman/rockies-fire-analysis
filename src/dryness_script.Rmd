...
make_roc <- function(options) {
  ...
  wbdata <- percentiles %>%
    filter(window == window, maj_veg_cl == cover_class)
  ...
  png(filename = paste0("../img/", region_name, "-", cover_class, "-", window, "-days.png"), width = 800, height = 800)
  par(pty = "s") ## pty sets the aspect ratio of the plot region. Two options:
  ##                "s" - creates a square plotting region
  ##                "m" - (the default) creates a maximal plotting region
  par(bg="white")
  
  plot.roc(col="pink",rd_roc, lwd=4, print.auc=FALSE, legacy.axes = TRUE, xlab="False Positive Percentage", ylab="True Postive Percentage", main = paste(region_name, "Forest ", window, " days"))
  ...
  roc_table <- tribble(~window, ~maj_veg_cl, ~var, ~auc, ~auc20, ~auc10)
  for (var in vars) {
    roc_table <- roc_table %>% add_row(window = window, maj_veg_cl = cover_class, var = var, auc = auc0(var), auclb = auc0lb(var), aubub = aub0ub(var), auc20 = auc20(var), auc10 = auc10(var))
  }
...
```{r ecdf-regression2-d-non_forest}
ecdf_fn <- ecdf(ignitions_non_forest$D)
summary(ecdf_fn)
...
regression_formulas <- regression_formulas %>%
    add_row(var = "D", order = "2", cover = "non_forest", formula = regression2_formula(b0, b1, b2), r2 = r2)
```
...
```{r ecdf-regression2-vpd-non_forest}
ecdf_fn <- ecdf(ignitions_non_forest$VPD)
summary(ecdf_fn)
...
regression_formulas <- regression_formulas %>%
    add_row(var = "VPD", order = "2", cover = "non_forest", formula = regression2_formula(b0, b1, b2), r2 = r2)
```
