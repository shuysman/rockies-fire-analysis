...
mtbs_poly_sfc <- st_read(mtbs_poly) %>%
...
basemap <- get_stadiamap(bbox = bbox, zoom = 7, maptype = "stamen_toner")

ggmap(basemap) +
  geom_sf(data = ecoregion, inherit.aes = FALSE, alpha = 0.15, fill = "blue") +
  geom_sf(data = mtbs_poly_sfc, aes(fill = maj_veg_cl), alpha = 0.75, inherit.aes = FALSE) +
  scale_fill_manual(values = c("darkolivegreen4", "khaki")) +
  labs(x = "Longitude",
       y = "Latitude",
       fill = "Cover",
...
```{r roc_analysis, fig.width = 6, fig.height = 6, include = TRUE, eval = T, message = T, echo = T}
...
make_roc <- function(options) {
  ...
  awssm_roc<-roc(wbdata$fire,wbdata$AWSSM, plot=FALSE)
  rain_roc<-roc(wbdata$fire,wbdata$RAIN, plot=FALSE)
  aet_roc<-roc(wbdata$fire,wbdata$AET, plot=FALSE)
  d_roc<-roc(wbdata$fire,wbdata$D, plot=FALSE)

  png(filename = paste0("img/", region_name, "-", cover_class, "-", roll_window, "-days.png"), width = 800, height = 800)
  ...
  plot.roc(add=TRUE,col="magenta",awssm_roc, main="awssm", lwd=4, print.auc=FALSE)
...
### Window vs AUC
``` {r auc-window, fig.height = 12, fig.width = 8}
...
annotate_figure(window_plot, top = text_grob("Rolling window width vs. (p)AUC", size = 18))
...
<!-- xcann<-subset(ignitions_non_forest,select=c("T","SOIL","GDD","AWSSM","AET","D","RD","VPD")) -->
...
ecdf_fn <- ecdf(non_forest_data$D)
summary(ecdf_fn)
...
regression_formulas <- regression_formulas %>%
    add_row(var = "D", order = "3", cover = "non_forest", formula = regression3_formula(b0, b1, b2, b3), r2 = r2)
```
