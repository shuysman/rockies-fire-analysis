...
percentiles <- mclapply(windows,
                        FUN = \(window) {
                          ...
                          wbdata_percentiles <- 
                            ...
                            filter(Date <= Ig_Date,
                                   yday(Date) >=
                                     case_when(maj_veg_cl == "forest" ~ forest_start,
                                               maj_veg_cl == "non_forest" ~ non_forest_start),  ## Filter to fire season
                                   yday(Date) <=
                                     case_when(maj_veg_cl == "forest" ~ forest_end,
                                               maj_veg_cl == "non_forest" ~ non_forest_end)) %>%
                            drop_na(vars) %>%
                            mutate(across(vars,
                                          my_percent_rank)) %>%
