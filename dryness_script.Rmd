...
master <- read_csv("SR_fire_points_extracted.csv") %>%
    ...
    rename("maj_veg_cl" = LF20_EVT_5) %>% #maj_class is the majority of pixels classification from Landfire 30m pixels in burned aresa
...
wbdata <- wbdata %>%
    mutate(Event_ID = str_extract(PATH, '(?<=ap23/).*(?=\\.csv)')) %>% ### Need to fix this Regex later
    group_by(Event_ID) %>%
    mutate(RD = 100 - RH,
...
wbdata_smoothed <- select(wbdata, c(Event_ID, Date, RD, VPD, T, SOIL, AWSSM, RAIN, AET, D, GDD)) %>%
    ## mutate_at(vars[1:5],
...
wbdata_smoothed <- left_join(wbdata_smoothed, select(master,
                                                     Event_ID, Ig_Date, maj_veg_cl),
                             by = "Event_ID") %>%
    mutate(fire = if_else(Date == Ig_Date,
                          1,
                          0)) 
