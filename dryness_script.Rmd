...
my_percent_rank <- function(x) {
    x_no_zeroes <- subset(x, x != 0)
    return(percent_rank(x_no_zeroes))
}
...
```{r rolling-windows, fig.width = 5, fig.height = 5, message = F, eval = T}
### TODO
...
```{r auc-windows-plots, fig.height = 8, fig.width = 8, eval = T}
...
```{r fire-seasons, fig.width = 5, fig.height = 5, message = F, eval = T}
### TODO
### Refactor this so its not copy pasted from above
...
```{r auc-quants-plots, fig.height = 8, fig.width = 8, eval = T}
forest_roc_table_full %>% pivot_longer(c(auc, auc20, auc10)) %>%
    ggplot() +
