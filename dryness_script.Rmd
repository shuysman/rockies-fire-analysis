...
settings <- tribble(
    ~region, ~params,
    "Southern Rockies", list(data_dir =  "/home/steve/OneDrive/northern-rockies-dryness",
...
##                                 "/home/steve/apr23_fire_points",
    ...
    "Middle Rockies", list(data_dir =  "/home/steve/OneDrive/northern-rockies-dryness",
...
knitr::opts_chunk$set(
  fig.path = paste0(data_dir, "/img/", str_replace_all(region_name, " ", "_"), "/")
)
```
...
ecoregions <- st_transform(read_sf("/home/steve/OneDrive/northern-rockies-dryness/data/us_eco_l3/us_eco_l3.shp"), crs = 4326)
ecoregion <- ecoregions %>% filter(US_L3NAME == region_name)

bbox_ecoregion <- st_bbox(ecoregion)
bbox <- c(left = bbox_ecoregion$xmin[[1]], bottom = bbox_ecoregion$ymin[[1]], right = bbox_ecoregion$xmax[[1]], top = bbox_ecoregion$ymax[[1]])

basemap <- get_stadiamap(bbox = bbox, zoom = 7, maptype = "stamen_terrain")

ggmap(basemap) +
    geom_sf(data = ecoregion, inherit.aes = FALSE, alpha = 0.15, fill = "red") +
...
wbdata_percentiles_forest <- select(wbdata_smoothed, c(Event_ID, Ig_Date, maj_veg_cl, Acres, fire, Date, vars)) %>%
    ...
    collect() %>%
    drop_na(vars) %>%
    ##mutate(D = case_when(D == 0 ~ NA, TRUE ~ D)) %>%
    mutate(across(vars,
                  percent_rank))
...
wbdata_percentiles_forest %>%
  filter(Event_ID == "UT3778610952720020601", year(Date) == 2000) %>%
  pivot_longer(vars) %>%
  filter(name == "D") %>%
...
```{r rolling-windows, fig.width = 5, fig.height = 5, message = F, eval = F}
### TODO
...
```{r auc-windows-plots, fig.height = 8, fig.width = 8, eval = F}
...
```{r fire-seasons-quants, fig.width = 5, fig.height = 5, message = F, eval = F}
### TODO
### Refactor this so its not copy pasted from above

fire_season_quants <- c(1, .95, .9, .85, .8, .75)

forest_roc_table_full <- tribble(~quant, ~var, ~auc, ~auc20, ~auc10)
nonforest_roc_table_full <- tribble(~quant, ~var, ~auc, ~auc20, ~auc10)

for (q in fire_season_quants) {    
    cluster_copy(cluster, "q")

    forest<-subset(master_c, maj_veg_cl=="forest")
    forest_q<-quantile(forest$doy,c(1 - q, q))#90% of data fall within this range 0.05-0.95# 0.27 to 0.75 or 0,1 to determine min max

    nonforest<-subset(master_c, maj_veg_cl=="non-forest")
    nonforest_q<-quantile(nonforest$doy,c(1 - q, q))#90% of data fall within this range 0.05-0.95# 0.27 to 0.75 or 0,1 to determine min
   ...
   png(filename = paste0("img/", region_name, "-forest-", q, "-quant-", window, "-days.png"), width = 800, height = 800)
    par(pty = "s") ## pty sets the aspect ratio of the plot region. Two options:
    ##                "s" - creates a square plotting region
    ##                "m" - (the default) creates a maximal plotting region
    par(bg="white")
    plot.roc(col="pink",forest_rd_roc, lwd=4, print.auc=FALSE, main = paste(region_name, "Forest ", window, " days"))
    plot.roc(add=TRUE,col="dark blue",forest_vpd_roc, main="vpd", lwd=4, print.auc=FALSE)
    plot.roc(add=TRUE,col="green",forest_t_roc, main="temp", lwd=4, print.auc=FALSE)
    plot.roc(add=TRUE,col="red",forest_soil_roc, main="soil", lwd=4, print.auc=FALSE)
    plot.roc(add=TRUE,col="brown",forest_gdd_roc, main="gdd", lwd=4, print.auc=FALSE)
    ...
    for (var in vars) {
        forest_roc_table_full <- forest_roc_table_full %>% add_row(quant = q, var = var, auc = auc0f(tolower(var)), auc20 = auc20f(tolower(var)), auc10 = auc10f(tolower(var)))
    }
    ...
    png(filename = paste0("img/", region_name, "-nonforest-", q, "-quant-", window, "-days.png"), width = 800, height = 800)
    par(pty = "s") ## pty sets the aspect ratio of the plot region. Two options:
    ##                "s" - creates a square plotting region
    ##                "m" - (the default) creates a maximal plotting region
    par(bg="white")
    plot.roc(col="pink",nonforest_rd_roc, lwd=4, print.auc=FALSE, main = paste(region_name, "Nonforest ", window, " days"))
    plot.roc(add=TRUE,col="dark blue",nonforest_vpd_roc, main="vpd", lwd=4, print.auc=FALSE)
    plot.roc(add=TRUE,col="green",nonforest_t_roc, main="temp", lwd=4, print.auc=FALSE)
    plot.roc(add=TRUE,col="red",nonforest_soil_roc, main="soil", lwd=4, print.auc=FALSE)
    plot.roc(add=TRUE,col="brown",nonforest_gdd_roc, main="gdd", lwd=4, print.auc=FALSE)
    ...
    for (var in vars) {
        nonforest_roc_table_full <- nonforest_roc_table_full %>% add_row(quant = q, var = var, auc = auc0nf(tolower(var)), auc20 = auc20nf(tolower(var)), auc10 = auc10nf(tolower(var)))
    }
}
...
```{r auc-quants-plots, fig.height = 8, fig.width = 8, eval = F}
forest_roc_table_full %>% pivot_longer(c(auc, auc20, auc10)) %>%
    ggplot() +
    geom_line(mapping = aes(x = quant, y = value, color = var), lwd=1) +
    facet_wrap(~name, ncol = 1, scales = "free") +
    ggtitle(paste(region_name, "Forest AUC and pAUC")) +
    plot_theme + theme(legend.position="bottom")
...
forest_t_roc<-roc(wbdata_percentiles_forest$fire,wbdata_percentiles_forest$T, plot=FALSE, legacy.axes=TRUE, percent=TRUE, col="#377eb8", lwd=4, print.auc=TRUE)
...
##                "m" - (the default) creates a maximal plotting region
par(bg="white")
plot.roc(col="pink",forest_rd_roc, lwd=4, print.auc=FALSE, legacy.axes = TRUE, xlab="False Positive Percentage", ylab="True Postive Percentage", main = paste(region_name, "Forest ", window, " days"))
plot.roc(add=TRUE,col="dark blue",forest_vpd_roc, main="vpd", lwd=4, print.auc=FALSE)
plot.roc(add=TRUE,col="green",forest_t_roc, main="temp", lwd=4, print.auc=FALSE)
plot.roc(add=TRUE,col="red",forest_soil_roc, main="soil", lwd=4, print.auc=FALSE)
plot.roc(add=TRUE,col="brown",forest_gdd_roc, main="gdd", lwd=4, print.auc=FALSE)
...
for (var in vars) {
    forest_roc_table <- forest_roc_table %>% add_row(window = window, var = var, auc = auc0f(tolower(var)), auc20 = auc20f(tolower(var)), auc10 = auc10f(tolower(var)))
}

knitr::kable(forest_roc_table)
```
...
nonforest_t_roc<-roc(wbdata_percentiles_nonforest$fire,wbdata_percentiles_nonforest$T, plot=FALSE, legacy.axes=TRUE, percent=TRUE, col="#377eb8", lwd=4, print.auc=TRUE)
...
##                "m" - (the default) creates a maximal plotting region
par(bg="white")
plot.roc(col="pink",nonforest_rd_roc, lwd=4, print.auc=FALSE, xlab="False Positive Percentage", ylab="True Postive Percentage", main = paste(region_name, "Nonforest ", window, " days"))
plot.roc(add=TRUE,col="dark blue",nonforest_vpd_roc, main="vpd", lwd=4, print.auc=FALSE)
plot.roc(add=TRUE,col="green",nonforest_t_roc, main="temp", lwd=4, print.auc=FALSE)
plot.roc(add=TRUE,col="red",nonforest_soil_roc, main="soil", lwd=4, print.auc=FALSE)
plot.roc(add=TRUE,col="brown",nonforest_gdd_roc, main="gdd", lwd=4, print.auc=FALSE)
...
for (var in vars) {
    nonforest_roc_table <- nonforest_roc_table %>% add_row(window = window, var = var, auc = auc0nf(tolower(var)), auc20 = auc20nf(tolower(var)), auc10 = auc10nf(tolower(var)))
}

knitr::kable(nonforest_roc_table)
```
...
```{r wb-vs-area, fig.width = 8, fig.height = 8, eval = F, include = F}
...
plot(ecdf_fn, xlab = "Percentile of Deficit", ylab = "Proportion of historical wildfires", main = "eCDF Nonforest Deficit, with ignition danger ratings", xlim = c(0,1))
points(y = estimated$proportions, x = estimated$quants, col = "blue")
points(y = estimated$predicted, x = estimated$quants, col = "red")
curve(exp(b0 + b1*x + b2*(x^2) + b3*(x^3)), add = TRUE, col = "red")
##curve(exp(predict(lm, newdata = data.frame(quants = x))), add = TRUE, col = "red")
...
```{r ecdf-regression3-rd-nonforest}
...
Estimated function predicting proportion of historic fires burning in nonforests at or below given percentile of `r window` day rolling mean of RD on day of ignition:
...
```{r log-model}
fire <- wbdata_percentiles_forest$fire
D <- wbdata_percentiles_forest$D

lm1 <- glm(fire ~ D, family = "binomial")
summary(lm1)

save(lm1, file = paste0("out/", str_replace(region_name, " ", "_"), "_lm.rda"))
